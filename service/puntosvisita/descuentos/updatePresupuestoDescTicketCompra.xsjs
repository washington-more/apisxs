...
function ActualizarVisita(empresa,codigo,cantidad) {
	...
	try {
		...
		query = 'update ' + dbname + '."@AB_DTBO" set "U_AB_DTBP"="U_AB_DTBP"-'+cantidad+' ';
		var where = ' where "Code"=\'' + codigo + '\'  ';
		query += where;
...
try {
	...
	if (empId !== undefined && cod !== undefined && cant !== undefined) {
		conn = $.db.getConnection();

		//actualizar OV
		//var precbase = ActualizarPrecios(empId, itemcode, pricel, marg, comment, codal);
        var precbase = ActualizarVisita(empId, cod, cant);
