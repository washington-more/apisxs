...
function ActualizarVisita(empresa,cli,cod, nom,cant) {
	...
	try {
		...
		query = 'update ' + dbname + '."@AB_CMST" set "U_AB_MSST"="U_AB_MSST"-'+cant+' ';
		var where = ' where "U_AB_MSQC"=\'' + cli + '\' and "U_AB_MSCU"=\'' + cod +'\' and "U_AB_MSNU"=\'' + nom +'\' ';
		query += where;
...
try {
	...
	var nombre = $.request.parameters.get('nombre');
	var cantidad = $.request.parameters.get('cantidad');

	if (empId !== undefined && cliente !== undefined && codigo !== undefined && nombre !== undefined) {
		conn = $.db.getConnection();

		//actualizar OV
		//var precbase = ActualizarPrecios(empId, itemcode, pricel, marg, comment, codal);
        var precbase = ActualizarVisita(empId, cliente, codigo, nombre,cantidad);
