...
try{
	...
	if (empId !== undefined)
	{
	   ...
	   // select "U_AB_MNF9" as "codigo","U_AB_MNDS" as "nombre" from "MB_JUNIO"."@AB_MNCD" where "U_AB_MNTD" in ('NC')
        // query = 'select "U_AB_MNF9" as "codigo", "U_AB_MNDS" as "nombre" from '+dbname+'."@AB_MNCD" where "U_AB_MNTD" in (\'NC\')';
        query = 'select "Code" as "codigo", "Name" as "nombre" from '+dbname+'."@SYP_FECAT09"';
