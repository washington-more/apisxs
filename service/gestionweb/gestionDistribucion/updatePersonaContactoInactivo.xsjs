...
function ActualizarVisita(empresa,vendedor, ruta) {
	...
	try {
		...
		query = 'update ' + dbname + '.OCPR set "U_AB_PCAC"=\'N\' ';
...
try {
	...
	var vend = $.request.parameters.get('vend');
	var rut = $.request.parameters.get('rut');

	if (empId !== undefined && vend !== undefined && rut !== undefined) {
		conn = $.db.getConnection();

        var precbase = ActualizarVisita(empId, vend, rut);
