...
function ObtenerLineas(clave, database){
	...
	'18.00 as "tasaigv",case when T1."DiscPrcnt"=100 then cast(ROUND(((ROUND((T1."PriceBefDi"),2)*T1."Quantity")*18)/100,2) as varchar) else cast(ROUND(T1."VatSum",2) as varchar) end as "importeigv", '+
	'case when T1."DiscPrcnt"=100 then \'15\' else \'10\' end as "codigorazonexoneracion",case when T1."DiscPrcnt"=100 then ROUND((ROUND((T1."PriceBefDi"),2)*T1."Quantity"),2) else T1."LineTotal" end as "importetotalsinimpuesto", '+
...
try{
	...
	'ifnull((select cast(ROUND(sum(((T3."PriceBefDi"*T3."Quantity")*18)/100),2) as varchar) from '+dbname+'.INV1 T3 where T3."DocEntry"=T0."DocEntry" and T3."DiscPrcnt"=100),\'0.00\') '+
