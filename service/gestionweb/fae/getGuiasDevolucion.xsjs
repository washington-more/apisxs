...
'from '+database+'.ORPD T0 inner join '+database+'.RPD1 T1 on T0."DocEntry"=T1."DocEntry" '+
...
try{
	...
	'ROUND( '+
	'(select sum(S0."Quantity"*case when S1."SWeight1"=0 then 1 else S1."SWeight1" end) from '+dbname+'.RPD1 S0 inner join '+dbname+'.OITM S1 on S0."ItemCode"=S1."ItemCode" where S0."DocEntry"=T0."DocEntry") '+
	',2) as "pesobrutototalbienes",\'KGM\' as "unidadmedidapesobruto",\'01\' as "modalidadtraslado", '+
	'T0."U_SYP_MDRT" as "numeroructransportista", '+
	'\'6\' as "tipodocumentotransportista", '+
	'T0."U_SYP_MDNT" as "razonsocialtransportista", '+
	'(select S0."Code" from '+dbname+'."@SYP_CONDUC" S0 where S0."U_SYP_CHLI"=T0."U_SYP_MDFC") as "numerodocumentoconductor",\'1\' as "tipodocumentoconductor", '+
	'T3."ZipCodeB" as "ubigeoptollegada",T3."StreetB" as "direccionptollegada", '+
	...
	'TO_VARCHAR (TO_DATE(ADD_DAYS(T0."DocDueDate",1)), \'YYYY-MM-DD\') as "fechaentregabienes",\'true\' as "indregvehiculoycond",T0."U_SYP_MDCI" as "numeroregistromtc", '+
	'T0."U_SYP_MDVC" as "numeroplacavehiculoprin",(select S0."U_SYP_CHNO" from '+dbname+'."@SYP_CONDUC" S0 where S0."U_SYP_CHLI"=T0."U_SYP_MDFC") as "nombreconductor", '+
	'(select S0."U_AB_CHAP" from '+dbname+'."@SYP_CONDUC" S0 where S0."U_SYP_CHLI"=T0."U_SYP_MDFC") as "apellidoconductor", '+
	'T0."U_SYP_MDFC" as "numerolicencia", '+
	'case when T2."U_SYP_BPTD"=\'6\' then \'01\' else \'03\' end as "codigodocumentodocrel", '+
	'T0."U_SYP_MDSO"||\'-\'||T0."U_SYP_MDCO" as "numerodocumentodocrel",1 as "ordendocrel", '+
	...
	'inner join '+dbname+'.RPD12 T3 on T0."DocEntry"=T3."DocEntry" '+
