...
function ActualizarComprometidos(empresa,numero, est){
    ...
    try{
        ...
        query = 'update '+empresa+'.OINV '+
        'set "U_SYP_MOTNCND"=\''+est+'\' '+
        ' where "DocEntry"='+numero+' ';
...
try{
	...
	var motivo = $.request.parameters.get('motivo');
	
	if (docentry !== undefined && motivo !== undefined )
	{
	    var dbname = functions.GetDataBase(empId);
	    conn = $.db.getConnection();
 
	    //actualizar OV
	    var actualizar = ActualizarComprometidos(dbname,docentry, motivo);
	        
	    if(actualizar === Constants.MESSAGE_SUCCESS){
            objType = Constants.SUCCESS_MESSAGE_RESPONSE;
	        objResult = functions.CreateJSONMessage(docentry, Constants.MESSAGE_SUCCESS);
	        conn.commit();
        }else{
            conn.rollback();
