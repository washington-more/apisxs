...
try{
	...
	{ 
	    ...
	    // nuevo modificacon, nueva columna despues de la salida pedido de geral
	    // T0."U_SYP_MDSD"||\'-\'||T0."U_SYP_MDCD" as "Documento"  
        query = 'select T0."DocEntry",T0."DocNum","PrcName" as "Sucursal",T0."CardCode" as "CodCliente",T0."CardName" as "Cliente", '+
        'T2."SlpName" as "Vendedor",T4."Name" as "Ruta",T5."PymntGroup" as "Condicion",T0."DocTotal"-T0."PaidToDate" as "Saldo", '+
        'case when T0."GroupNum"=-1 then (T0."DocTotal"-T0."PaidToDate")-T0."U_AB_DMMR" else 0 end as "Pago",T0."U_AB_DMMR" as "Rechazo",\'Efectivo\' as "TipoPago", '+
