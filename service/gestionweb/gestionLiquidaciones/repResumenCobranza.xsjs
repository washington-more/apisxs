...
try{
    ...
    conso = $.request.parameters.get('conso');
    //fecha = $.request.parameters.get('fecha');
    //placa = $.request.parameters.get('placa');

	if (empId !== undefined)
	{
	    var dbname = functions.GetDataBase(empId);
    	var conn = $.hdb.getConnection();
        query = conn.loadProcedure(dbname, "ABJ_LISTAR_RES_LIQ"); 
        var params = query({ "Consolidado": conso});
