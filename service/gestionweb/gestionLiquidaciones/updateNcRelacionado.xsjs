...
function ActualizarVisita(empresa,clave,claveNc) {
	...
	try {
		...
		query = 'update ' + dbname + '.OINV set "U_AB_DMNR"=' + claveNc + ' ';
...
try {
	...
	var claveNC = $.request.parameters.get('claveNC');

	if (empId !== undefined && docentry !== undefined && claveNC !== undefined) {
		conn = $.db.getConnection();

        var precbase = ActualizarVisita(empId, docentry, claveNC);
